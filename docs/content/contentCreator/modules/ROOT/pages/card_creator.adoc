...
Below that are 3 more buttons, "Deck Properties", "Deck Stats", and "Card Style". Clicking on any of these will open their respective window.
...
The "Deck Properties" section is where you can see, and set, various properties for the deck. This includes the name of the deck, a link to the icon of the class, and various links to different class tokens, allowing you to add custom images to cards (this is explained in more detail in the <<basic-actions, Basic Actions>>, and the <<custom-actions, Custom Actions>> section). The custom class tokens can also be adjusted in scale, each having two separate scale properties, one for inline size and one for main action as will be explained in the <<syntax, Syntax>> section.
...
When using the color picker, pressing the label at the bottom where it says `RGB` will cycle through different color definitions: `RGB` -> `HSL` -> `HEX` -> `RGB`. This can be useful depending on which color format you prefer to work with.
...
An extension of the mandatory block is the <<mode-blocks, Mode block>>.
...
You may also use some letters for custom colored hexes, which you may color using css styling as used in the example. These letters include 'd', 'h', 'f' and 'c'.
...
Inside text-based blocks such as `custom`, you can also use a few special formatting helpers. `%nbsp:X%` inserts multiple spaces, where `X` is the number of spaces to add, and `%lf%` inserts a line break. These can be useful when you need more precise control over formatting inside a text block.
